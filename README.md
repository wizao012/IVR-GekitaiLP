...
コンテナ `GTM-KJFDVJR3` を `index.html` と `thank-you.html` の両方に設置済みです。
通常のスクリプトをhead上部、noscript iframeをbody直後に配置しています。
提供されたコードのMarkdownリンク表記とエスケープを通常のHTMLに直して組み込んでいます。
コンテナ内のタグ・トリガー設定や公開状態、実際の計測受信は今回確認していません。


## Meta広告イベントの直接送信（今回追加）

MetaピクセルID：`931880797549354`
公開先：`https://wizao012.github.io/IVR-GekitaiLP/`

- LPと完了画面にベースコードを直接配置し、PageViewを送ります。
- `meta-events.js` が電話リンクのクリックで Contact を送ります（通話成立の計測ではありません）。
- フォームのWebhook成功後にだけ Lead を送ります。完了画面の直接アクセス・再読み込みではLeadを送りません。
- Leadに送信識別子をeventIDとして付け、同じページ内で同じ識別子を重複送信しない処理を入れています。
- `trackSingle` で指定のピクセルだけを送信対象にしています。
- イベント追加パラメータは `content_name=撃退くん` と `contact_method=phone/form` です。フォームの氏名・メール・電話番号・自由記述はイベント追加パラメータに入れていません。
- 送信後はブラウザに600msの送出時間を設けてから完了画面へ移動します。Metaの受信完了を保証する待機ではありません。広告ブロッカー等でMeta通信が止まる場合は受信できません。

### 公開時の作業

1. このフォルダの中身をGitHubの公開元へ上書きしてください。特に `index.html`、`thank-you.html`、`form.js`、新規 `meta-events.js` が更新対象です。
2. GTMやMetaの自動イベント設定で同じPageView・Contact・Leadを送る設定を追加済みなら、二重発火しないよう重複分を停止してください。今回の実装はGTM内のMetaタグを必要としません。GTMコンテナ自体は維持しています。
3. Metaイベントマネージャの「テストイベント」で公開LPを開き、PageView、電話クリックのContact、送信成功のLeadを確認します。送信テストはZapierにも実際に届きます。
4. 広告の計測・最適化対象として該当イベントまたはLP条件で絞ったカスタムコンバージョンを選択してください。イベント受信と、広告成果としてのアトリビューションは別の確認です。

ローカル模擬テストでイベントの分岐・重複抑止・ピクセルID・既存Webhook形式を確認済みです。GitHubへの反映、Metaの実受信、GTM管理画面の既存設定変更は今回行っていません。
